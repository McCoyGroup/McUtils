"""Extracted from NumputilsTests.test_RenderMatrix via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest NumputilsTests.test_RenderMatrix"""
...
class NumputilsTests(TestCase):
    ...
    def test_RenderMatrix(self):
        ...
        rm = nput.render_matrix(bbox=[[0, 2], [-2, 2], [-2, 2]], view_position=[0, 1, 1], view_center=[0, 1, 0], view_angle=np.pi)
        print(rm)
