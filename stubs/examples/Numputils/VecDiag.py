"""Extracted from NumputilsTests.test_VecDiag via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest NumputilsTests.test_VecDiag"""
...
class NumputilsTests(TestCase):
    ...
    def test_VecDiag(self):
        ...
        self.assertEquals(diag_vec[0, 2, 1, 1, 1], ugh[0, 2, 1])
        diag_mats = vec_tensordiag(ugh, extra_dims=2, axis=1)
        self.assertEquals(diag_mats.shape, (3, 7, 7, 7, 5))
        self.assertTrue(np.allclose(diag_mats[0, 1, 1, 1], ugh[0, 1]))
