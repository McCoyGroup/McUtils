"""Extracted from ConverterTest.test_CartExpanded via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ConverterTest.test_CartExpanded"""
...
class ConverterTest(TestCase):
    ...
    def test_CartExpanded(self):
        expansion = np.array([[1 / np.sqrt(6), np.sqrt(2 / 3), 1 / np.sqrt(6)], [1 / np.sqrt(3), -(1 / np.sqrt(3)), 1 / np.sqrt(3)], [1 / np.sqrt(2), 0, -(1 / np.sqrt(2))]])
        system = CoordinateSystem(basis=CartesianCoordinates3D, matrix=expansion)
        disp = system.displacement(0.1)
        self.assertEquals(disp, 0.1)
