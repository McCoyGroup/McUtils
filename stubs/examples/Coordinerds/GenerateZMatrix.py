"""Extracted from ConverterTest.test_GenerateZMatrix via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ConverterTest.test_GenerateZMatrix"""
...
class ConverterTest(TestCase):
    ...
    def test_GenerateZMatrix(self):
        zmat = [[0, -1, -1, -1], [1, 0, -1, -1], [2, 0, 1, -1], [3, 1, 2, 0]]
        coords = extract_zmatrix_internals(zmat)
        print(coords)
        for zm in enumerate_zmatrices(coords):
            print(zm)
