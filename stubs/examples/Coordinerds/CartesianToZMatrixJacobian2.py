"""Extracted from ConverterTest.test_CartesianToZMatrixJacobian2 via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ConverterTest.test_CartesianToZMatrixJacobian2"""
...
class ConverterTest(TestCase):
    ...
    def test_CartesianToZMatrixJacobian2(self):
        ...
        self.assertEquals(njacob.shape, (10 * 3, 10 * 3, 10 - 1, 3))
        jacob = coord_set.jacobian(ZMatrixCoordinates, 2, stencil=5)
        self.assertEquals(jacob.shape, (10, 3, 10, 3, 10 - 1, 3))
        njacob = njacob.reshape((10, 3, 10, 3, 10 - 1, 3))
        diffs = njacob - jacob
        bleh_bleh = np.where(np.round(diffs, 3) != 0.0)
        self.assertTrue(np.allclose(diffs, 0.0, atol=0.001), msg='wat: {}'.format(np.max(np.abs(np.round(diffs, 3)))))
