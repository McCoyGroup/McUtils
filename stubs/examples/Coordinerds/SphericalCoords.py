"""Extracted from ConverterTest.test_SphericalCoords via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ConverterTest.test_SphericalCoords"""
...
class ConverterTest(TestCase):
    ...
    def test_SphericalCoords(self):
        ...
        self.assertAlmostEquals(np.sum(newnew - crds)[()], 0.0)
        self.assertAlmostEquals(np.sum(coord_set - old)[()], 0.0)
