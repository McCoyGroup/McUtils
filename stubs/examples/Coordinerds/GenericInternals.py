"""Extracted from ConverterTest.test_GenericInternals via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ConverterTest.test_GenericInternals"""
...
class ConverterTest(TestCase):
    ...
    def test_GenericInternals(self):
        ...
        coords = Molecule.from_file(os.path.join(test_root, 'nh3.fchk')).coords
        new_coords = CoordinateSet(coords).convert(GenericInternalCoordinates, specs=[(0, 1), (0, 2), (0, 3), (1, 0, 2), (1, 0, 3), (2, 0, 3)])
        new_coords = new_coords + np.array([0, 0, 0, 0.2, 0, 0])
        raise Exception(coords - new_coords.convert(CartesianCoordinates3D))
