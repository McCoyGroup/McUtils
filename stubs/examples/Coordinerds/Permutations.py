"""Extracted from ConverterTest.test_Permutations via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ConverterTest.test_Permutations"""
...
class ConverterTest(TestCase):
    ...
    def test_Permutations(self):
        print()
        coords = extract_zmatrix_internals([[0, -1, -1, -1], [1, 0, -1, -1], [2, 0, 1, -1], [3, 0, 2, 1], [4, 1, 3, 2]], canonicalize=True)
        self.assertEquals(coords, permute_internals(permute_internals(coords, [0, 2, 1, 4, 3]), np.argsort([0, 2, 1, 4, 3])))
