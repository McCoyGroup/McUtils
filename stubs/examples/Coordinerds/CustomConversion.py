"""Extracted from ConverterTest.test_CustomConversion via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ConverterTest.test_CustomConversion"""
...
class ConverterTest(TestCase):
    ...
    def test_CustomConversion(self):
        ...
        self.assertEquals(np.sum(crds + coord_set), 0.0)
        self.assertEquals(np.sum(coord_set - old), 0.0)
        self.assertAlmostEquals(np.sum(coord_set.jacobian(new)[:, 0].reshape(30, 30) + np.eye(30, 30)), 0.0)
