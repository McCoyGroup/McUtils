"""Extracted from ConverterTest.test_SimpleZMatrices via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ConverterTest.test_SimpleZMatrices"""
...
class ConverterTest(TestCase):
    ...
    def test_SimpleZMatrices(self):
        pprint.pprint(coordops.functionalized_zmatrix(coordops.chain_zmatrix(2), ethyl_positions=[0, 1]))
        pprint.pprint(coordops.functionalized_zmatrix(coordops.chain_zmatrix(3), {(1, 0, 2): coordops.functionalized_zmatrix(coordops.chain_zmatrix(2), ethyl_positions=[0, 1])}, methyl_positions=[2], ethyl_positions=[0]))
