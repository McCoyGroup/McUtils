"""Extracted from ZacharyTests.test_InternalReexpansions via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_InternalReexpansions"""
...
class ZacharyTests(TestCase):
    ...
    def test_InternalReexpansions(self):
        ...
        hess = hess @ hess.T
        sample_coords = [[0, 1], [0, 2], [0, 3], [0, 1, 2], [0, 1, 3], [1, 2, 3]]
        vals = []
        jacs = []
        jac_derivs = []
        ...
        new_surf = nput.tensor_reexpand([np.array(jacs), np.array(jac_derivs)], [grad, hess], 2)
