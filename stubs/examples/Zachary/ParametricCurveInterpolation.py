"""Extracted from ZacharyTests.test_ParametricCurveInterpolation via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_ParametricCurveInterpolation"""
...
class ZacharyTests(TestCase):
    ...
    def test_ParametricCurveInterpolation(self):
        ...
        vals = np.array([np.cos(grid), np.sin(grid), np.sin(grid) + np.cos(grid)]).T
