"""Extracted from ZacharyTests.test_ExpansionDerivs via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_ExpansionDerivs"""
...
class ZacharyTests(TestCase):
    ...
    def test_ExpansionDerivs(self):
        ...
        point = np.array([0.5], dtype=dtype)
        disp = 0.01
        ...
        def sin_xy_d(pt):
            return np.array([np.cos(pt[..., 0]) * np.sin(pt[..., 1]), np.sin(pt[..., 0]) * np.cos(pt[..., 1])]).T
        exp2D = FunctionExpansion.expand_function(sin_xy, np.array([0.5, -0.5]), function_shape=((2,), 0), order=7, stencil=11)
        self.assertLess(np.linalg.norm(exp2D.deriv()([exp2D.center, exp2D.center - disp, exp2D.center + disp]) - sin_xy_d(np.array([exp2D.center, exp2D.center - disp, exp2D.center + disp]))), 0.005)
