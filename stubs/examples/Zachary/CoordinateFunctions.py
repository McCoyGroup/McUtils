"""Extracted from ZacharyTests.test_CoordinateFunctions via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_CoordinateFunctions"""
...
class ZacharyTests(TestCase):
    ...
    def test_CoordinateFunctions(self):
        fun = (CoordinateFunction.morse((0, 1), re=1.8, a=1, de=10) + CoordinateFunction.morse((0, 2), re=1.8, a=1, de=10) / 100) * CoordinateFunction.sin((1, 0, 2), n=2)
        test_coords = np.random.normal(0, 1, size=(1000, 5, 3))
        r, morse_vals = fun(test_coords, reexpress=False, order=2)
        base_fig = plt.ListContourPlot(np.concatenate([r[0], morse_vals[0][..., np.newaxis]], axis=1), colorbar=True)
        plt.ScatterPlot(r[0][..., 0], r[0][..., 1], figure=base_fig).show()
