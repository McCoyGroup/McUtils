"""Extracted from ZacharyTests.test_FDDeriv2 via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_FDDeriv2"""
...
class ZacharyTests(TestCase):
    ...
    def test_FDDeriv2(self):
        ...
        self.assertEquals(diff_fun[0, :].shape, (1, 2, npts, npts))
        self.assertEquals(np.linalg.norm((diff_fun[:, :][1, 0] - diff_fun[1, 0]).flatten()), 0.0)
