"""Extracted from ZacharyTests.test_bin_gs via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_bin_gs"""
...
class ZacharyTests(TestCase):
    ...
    def test_bin_gs(self):
        ...
        ans = np.array([1.0, 3.5, 4.375, 2.1875, 0.273438, -0.0273438, 0.00683594, -0.00244141])
        self.assertAlmostEqual(np.round(np.linalg.norm(gbin - ans), 5), 0.0)
