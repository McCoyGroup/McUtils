"""Extracted from ZacharyTests.test_FDDeriv via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_FDDeriv"""
...
class ZacharyTests(TestCase):
    ...
    def test_FDDeriv(self):
        ...
        diff_fun = fdf(grid, mesh_spacing=0.1)
        d = np.array([-0.2, -0.1, 0, 0.1, 0.2])
        raw = np.array([finite_difference(c + d, np.sin(c + d), (1,), stencil=5, only_center=True, end_point_accuracy=0) for c in grid])
        self.assertLess(np.linalg.norm(diff_fun[0] - raw), 1e-05)
