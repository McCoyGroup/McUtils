"""Extracted from ZacharyTests.test_RBFTiming via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_RBFTiming"""
...
class ZacharyTests(TestCase):
    ...
    def test_RBFTiming(self):
        for npts in [300]:
            ...
            interp = RBFDInterpolator(pts, vals, dvals, d2vals, neighborhood_size=28, clustering_radius=0.05)
            from Peeves.Timer import Timer
            with BlockProfiler(name='{} pts:'.format(len(interp.grid))):
                interp(interp.grid[:25] + 0.2, deriv_order=0, neighbors=5)
