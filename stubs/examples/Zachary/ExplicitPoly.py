"""Extracted from ZacharyTests.test_ExplicitPoly via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_ExplicitPoly"""
...
class ZacharyTests(TestCase):
    ...
    def test_ExplicitPoly(self):
        a = FunctionExpansion.from_indices({(): 1, (0,): 2}, ndim=2)
        self.assertTrue(np.allclose(a([[[1, 2], [0.5, 2]], [[1, 3], [0.5, 3]], [[1, 4], [0.5, 4]]]), np.array([[3, 2], [3, 2], [3, 2]])))
