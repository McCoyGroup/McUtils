"""Extracted from ZacharyTests.test_RBFInterp1D via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_RBFInterp1D"""
...
class ZacharyTests(TestCase):
    ...
    def test_RBFInterp1D(self):
        ...
        interp = RBFDInterpolator(pts, vals, np.array([dvals_x]).T, np.reshape(d2vals_x, (npts, ndim, ndim)), kernel='thin_plate_spline')
        pts = interp.grid
        vals = interp(pts[:5], deriv_order=0, neighbors=5)
        true = np.product(np.sin(pts[:5]), axis=1)
        self.assertTrue(np.allclose(vals, true), msg='bad interpolation at interpolation points \n{} \nvs\n{}'.format(vals, true))
        dervs = interp(pts[:2], deriv_order=1, neighbors=5)[1]
        true = np.array([dvals_x]).T[:2]
        self.assertTrue(np.allclose(dervs, true), msg='bad deriv interpolation at interpolation points \n{} \nvs\n {}'.format(dervs, true))
        self.assertLess(np.linalg.norm(interp(pts[:2] + 0.05, deriv_order=0, neighbors=5) - np.product(np.sin(pts[:2]), axis=1)), 0.2)
