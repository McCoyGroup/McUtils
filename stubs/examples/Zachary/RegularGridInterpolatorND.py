"""Extracted from ZacharyTests.test_RegularGridInterpolatorND via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_RegularGridInterpolatorND"""
...
class ZacharyTests(TestCase):
    ...
    def test_RegularGridInterpolatorND(self):
        ...
        self.assertTrue(np.allclose(interp_vals, test_vals, atol=5e-05))
        deriv = reg_interp.derivative((2, 0, 0))
        test_deriv_vals = -np.sin(test_points[:, 0])
        interp_vals = deriv(test_points)
        self.assertTrue(np.allclose(interp_vals, test_deriv_vals, atol=0.005))
        cos_grid = np.linspace(0, 1, 8)
        sin_grid1 = np.linspace(0, 1, 11)
        sin_grid = np.linspace(0, 1, 12)
        grids = [sin_grid, sin_grid1, cos_grid]
        ...
        interp_vals = deriv(test_points)
        self.assertTrue(np.allclose(interp_vals, test_deriv_vals, atol=0.0005))
