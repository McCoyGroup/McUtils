"""Extracted from ZacharyTests.test_StructuredMesh via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_StructuredMesh"""
...
class ZacharyTests(TestCase):
    ...
    def test_StructuredMesh(self):
        mg = np.array(np.meshgrid(np.array([-1, -0.5, 0, 1]), np.array([-1, 0, 1]), indexing='ij'))
        regmesh = Mesh(mg)
