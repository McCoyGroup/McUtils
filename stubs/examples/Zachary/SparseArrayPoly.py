"""Extracted from ZacharyTests.test_SparseArrayPoly via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_SparseArrayPoly"""
...
class ZacharyTests(TestCase):
    ...
    def test_SparseArrayPoly(self):
        ...
        self.assertTrue(np.allclose((poly + poly).coeffs.asarray(), (doly + doly).coeffs))
        shape = (10, 4, 3, 7)
        n_els = 4
        np.random.seed(1)
        inds = np.unique(np.array([np.random.choice(x, n_els) for x in shape]).T, axis=0)
        vals = np.random.rand(len(inds))
        inds = inds.T
        ar2 = SparseArray.from_data((vals, inds), shape=shape)
        goly = DensePolynomial(ar2, stack_dim=1)
        dggy = DensePolynomial(ar2.asarray(), stack_dim=1)
        self.assertTrue(np.allclose((poly + goly).coeffs.asarray(), (doly + dggy).coeffs))
        self.assertTrue(np.allclose(poly.grad().coeffs.asarray(), doly.grad().coeffs))
        self.assertTrue(np.allclose((poly + goly).grad().coeffs.asarray(), (doly + dggy).grad().coeffs))
