"""Extracted from ZacharyTests.test_TensorExpressionEfficiency via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_TensorExpressionEfficiency"""
...
class ZacharyTests(TestCase):
    ...
    def test_TensorExpressionEfficiency(self):
        ...
        tet2 = TensorExpression.OuterTerm(TensorExpression.OuterPowerTerm(TensorExpression.ConstantArray([1, 2, 3], name='bloop'), 2), TensorExpression.ConstantArray([1, 2, 3], name='bloop').dQ())
