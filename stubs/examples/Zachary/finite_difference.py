"""Extracted from ZacharyTests.test_finite_difference via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_finite_difference"""
...
class ZacharyTests(TestCase):
    ...
    def test_finite_difference(self):
        ...
        deriv = finite_difference(sin_grid, sin_vals, 1)
        sin_grid = np.arange(0, 1, 0.001)
        sin_vals = np.sin(sin_grid)
        ...
        for ord in range(2, 10):
            n = 1
            sten = ord
            vals = finite_difference(sin_grid, sin_vals, n, stencil=sten, end_point_accuracy=0, mode='sparse')
        ...
        for ord in range(3, 10):
            n = 2
            sten = ord
            vals = finite_difference(sin_grid, sin_vals, n, stencil=sten, end_point_accuracy=0, mode='dense')
            errs = self.get_error(ref_vals, vals)
            if plot_error:
                self.plot_err(sin_grid, ref_vals, vals, errs)
            if print_error:
        ...
        for ord in range(4, 10):
            n = 3
            sten = ord
            vals = finite_difference(sin_grid, sin_vals, n, ord, stencil=sten, end_point_accuracy=2, mode='sparse')
            errs = self.get_error(ref_vals, vals)
            if plot_error:
                self.plot_err(sin_grid, ref_vals, vals, errs)
            if print_error:
