"""Extracted from ZacharyTests.test_IrregularInterpolator3D via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_IrregularInterpolator3D"""
...
class ZacharyTests(TestCase):
    ...
    def test_IrregularInterpolator3D(self):
        ...
        self.assertTrue(np.allclose(interp_vals, test_vals, atol=0.005), msg='max diff: {}'.format(np.max(np.abs(interp_vals - test_vals))))
