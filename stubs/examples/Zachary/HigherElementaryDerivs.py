"""Extracted from ZacharyTests.test_HigherElementaryDerivs via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_HigherElementaryDerivs"""
...
class ZacharyTests(TestCase):
    ...
    def test_HigherElementaryDerivs(self):
        ...
        np.testing.assert_allclose(fn.deriv(order=3)([[1, 2], [3, 4]]), fexpr.deriv(order=3)([[1, 2], [3, 4]]))
