"""Extracted from ZacharyTests.test_FiniteDifferenceParallelism via McUtils.Docs.ExamplesParser -- not the original file, and may reference test-only setup/state. Run with: python -m unittest ZacharyTests.test_FiniteDifferenceParallelism"""
...
class ZacharyTests(TestCase):
    ...
    def test_FiniteDifferenceParallelism(self):
        ...
        coords = np.array([[0.0, 0.0, 0.0], [re_1, 0.0, 0.0], np.dot(nput.rotation_matrix([0, 0, 1], b_e), [re_2, 0.0, 0.0])])
        ...
        kb = 729160000000000.0 / (2 * np.pi) * hz2h
        morse = self.harmonically_coupled_morse(De_1, a_1, re_1, De_2, a_2, re_2, kb, b_e)
        deriv_gen = FiniteDifferenceDerivative(morse, function_shape=((None, None), 0), mesh_spacing=0.001, stencil=9, logger=Logger(), parallelizer=MultiprocessingParallelizer()).derivatives(coords)
        with BlockProfiler('With parallelizer'):
            pot_derivs = deriv_gen.derivative_tensor([1, 2, 3, 4, 5, 6, 7])
        deriv_gen = FiniteDifferenceDerivative(morse, function_shape=((None, None), 0), mesh_spacing=0.001, logger=Logger(), stencil=9).derivatives(coords)
