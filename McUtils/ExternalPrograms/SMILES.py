...
class SMILESSupplier:
    def __init__(self, smiles_file, line_indices=None, size=int(1e3)):
    ...
    def __enter__(self):
        ...
        if self._call_depth == 1:
            ...
            if isinstance(self.line_indices, np.ndarray):
                ...
                self._offsets = np.load(self.line_indices)
            ...
            if block_size is None:
                ...
                if n + block_size >= self._max_offset:
                    ...
                    for m in range(block_size):
                        ...
                        blocks.append(self._consume_next(db))
                    return blocks
                else:
                    return [
                        self._consume_next(db)
...
def _consume_supplier_mp(smiles_file, consumer, line_offset, block_size):
    ...
    res = []
    with supplier:
        for smi in supplier.find_smi(0, block_size):
            subres = consumer(smi)
            if subres is not None: res.append(subres)
    return res

def consume_smiles(supplier:SMILESSupplier, consumer, pool=None, upto=None):
    if dev.is_int(pool):
    ...
    if pool is None:
        res = []
        with supplier:
            for smi in (supplier if upto is None else supplier.find_smi(0, block_size=upto)):
                subres = consumer(smi)
                if subres is not None: res.append(subres)
