...
from ..Scaffolding import LineIndexedSupplier
import numpy as np
import hashlib
import json
...
class SMILESSupplier(LineIndexedSupplier):
    ...
    STORED_FILE_NAME = "lines.smi"
    INDEX_FILE_NAME = "line_indices.npy"
    META_FILE_NAME = "meta.json"

    # Existing public constants remain valid.
    LISMI_SMI_MEMBER = STORED_FILE_NAME
    LISMI_IDX_MEMBER = INDEX_FILE_NAME
    LISMI_META_MEMBER = META_FILE_NAME
    ...
    def __init__(
        ...
        if line_parser is not None:
            deserialization_function = line_parser
        self._line_parser = deserialization_function
        super().__init__(
            smiles_file,
            line_indices=line_indices,
            name=name,
            size=size,
            managed_streams=managed_streams,
            deserialization_function=deserialization_function,
            metadata_arrays=metadata_arrays,
            stored_file_name=stored_file_name,
        )
        # Preserve both the old stream attribute and parser attributes.
        self.smi = self.data
        if isinstance(self.split_char, str) and self._binary:
            self.split_char = self.split_char.encode(self._encoding)
        if self.deserialization_function is None:
            self.deserialization_function = self._default_parser
            self._deserialization_function = self.deserialization_function
        self.line_parser = self.deserialization_function

    @classmethod
    def _split_smi(
        cls, line: bytes, encoding="utf-8", split_idx=0, split_char=None, maxsplit=-1
    ):
        if split_char is None:
            return line.split(maxsplit=maxsplit)[split_idx].strip().decode(encoding)
        return (
            line.split(sep=split_char, maxsplit=maxsplit)[split_idx]
            .strip()
            .decode(encoding)
        )

    def _default_parser(self, line):
        return self._split_smi(
            line,
    ...
    def to_mp_state(self):
        # Preserve the historical state layout used by _consume_supplier_mp.
        return (
            self.smi._input,
            self.name,
            self.split_idx,
    ...
    def from_mp_state(cls, state, line_indices=None, **extra):
        smiles_file, name, split_idx, split_char, line_parser = state
        return cls(
            smiles_file,
            line_indices=line_indices,
            name=name,
    ...
    def find_smi(self, n, block_size=None, include_metadata=None):
        ...
        return self.find_object(
            n, block_size=block_size, include_metadata=include_metadata
        )

    def write_database_index(self, target, **options):
    ...
    def build_line_index_smiles_database_from_source(
        ...
        if isinstance(supplier_or_smiles_file, cls):
            split_idx = supplier_or_smiles_file.split_idx
            split_char = supplier_or_smiles_file.split_char
        if isinstance(split_char, bytes):
            split_char = split_char.decode("utf-8")
        return cls.build_jump_database(
            ...
            overwrite=overwrite,
            stored_file_name=stored_file_name,
            extra_metadata={"split_idx": split_idx, "split_char": split_char},
        )
